/****************************************************************************
 *
 *						b_spline_smooth.c (mex file)
 *
 *
 *  B-spline data smoothing function, using the Generalized Cross-Validation
 *  and Mean-Squared Prediction Error Criteria of Craven & Wahba (1979) enabling 
 *  the estimation of k-th order derivatives. (See functions gcvspl and splder
 *	below). 
 *
 ****************************************************************************
 *
 ...
 * Where
 *
 *		x = Nx1 double array of independent variables
 *
 *		y = Nx1 double array of data to smooth and interpolate
 *
 *		x_new = Mx1 double array "new" independent varibales or interpoloation 
 *				points. x[0] <= x_new[i] <= x[N] for all i =1,...,M. 
 *
 *		w = Nx1 double array of weight factors, where each w[i] corresponds
 ...
 *
 *		half_order = (double) half order of the required B-spline. Must be 
 ...
 *					  
 *
 *		deriv_order = (double) the order of the desired derivative. This must 
 ...
 * Notes: 
 *
 * If call option 1 is used then it is assumed that the error variance is a
 ...
 * Revision History:
 *
 * 1.) Woltring 1985. Original FORTRAN code. 
 *
...
#include "mex.h"		/* Standard mex file header							*/
...
void mexFunction( 
				 ...
				 mxArray *plhs[],		/* Lefthand side arguments			*/ 
				 int nrhs,				/* # of right-hand side arguments	*/
				 const mxArray *prhs[]	/* Righthand side arguments			*/
				 )
{
	...
	int num_out;			/* Number of output points						*/ 

	double *x;				/* Independent variables (strictly increasing)	*/

	double *y;				/* Input data to be smoothed					*/

	double *x_new;			/* Set of output independent variables			*/

	double *y_new;			/* Output data - smoothed data					*/

	double *weights;		/* Weight factor array							*/

	double var;				/* Error variance								*/

	double *var_out_pr;		/* Pointer to lhs var output					*/

	double *spline_coeff;	/* Spline Coeff									*/

	double *work_vector;	/* Work vector									*/

	double *work_vector2;	/* Second work vector							*/

	int half_order;			/* Half oder of spline							*/

	int deriv_order;		/* Order of derivative	 >= 0, <= 2*half_order	*/

	int ier;				/* Error parameter								*/

	int loc;				/* Location indicator for x_new[i]				*/
	...
	if ( nrhs == 6 && nlhs > 2 ) {
		...
		mexErrMsgTxt( "Too many lhs arguments\n" );
	...
	if ( nlhs < 1 ) {
		...
		mexErrMsgTxt( "Too few lhs arguments \n" );
		
	}

	/* Get first rhs argument */
	if ( !mxIsDouble( prhs[0] ) || mxIsComplex( prhs[0] ) ) {

		mexErrMsgTxt( "1st rhs argument of incorrect type\n" );

	}

	if ( mxGetN( prhs[0] ) != 1 ) {

		mexErrMsgTxt( "1st rhs (input) argument size incorrect\n" );
	...
	/* Verify that data in x are strictly increasing */
	for ( i = 1; i < num_points; i++ ) {

		if ( x[i - 1] >= x[i] ) {


			mexErrMsgTxt( "1st rhs argument must be strictly increasing\n" );
	...
	/* Get 2nd rhs argument */
	if ( !mxIsDouble( prhs[1] ) || mxIsComplex( prhs[1] ) ) {

		mexErrMsgTxt( "2nd rhs argument of incorrect type\n" );

	}

	if ( mxGetM( prhs[1] )!= num_points || mxGetN( prhs[1] ) != 1 ) {


		mexErrMsgTxt( "2nd rhs argment size incorrect \n" );

	}

	y = mxGetPr( prhs[1] );

	/* Get 3rd rhs argument */
	if ( !mxIsDouble( prhs[2] ) || mxIsComplex( prhs[2] ) ) {

		mexErrMsgTxt( "3rd rhs argument of incorrect type\n" );

	}
	
	if ( mxGetN( prhs[2] ) != 1 ) {

		mexErrMsgTxt( "3rd rhs argument of incorrect size\n" );

	}

	num_out = mxGetM( prhs[2] );

	x_new = mxGetPr( prhs[2] );

	/* Get 4th rhs argument */
	if ( !mxIsDouble( prhs[3] ) || mxIsComplex( prhs[3] ) ) {

		mexErrMsgTxt( "4th rhs argument of incorrect type\n" );

	}

	if ( mxGetM( prhs[3] ) != num_points || mxGetN( prhs[3] ) != 1 ) {


		mexErrMsgTxt( "4th rhs argument of incorrect size\n" );
	...
	for ( i = 0; i < num_points; i++ ) {
		...
		if ( weights[i] <= 0 ) {
			...
			mexErrMsgTxt( "4th rhs argument, array values must be > 0 \n" );

		}

	}

	/* Get 5th rhs argument */
	if ( !mxIsDouble( prhs[4] ) || mxIsComplex( prhs[4] ) ) {

		mexErrMsgTxt( "5th rhs argument of incorrect type\n" );
	...
	if ( mxGetN( prhs[4] ) != 1 || mxGetM( prhs[4] ) != 1 ) {
		...
		mexErrMsgTxt( "5th rhs argument of incorrect size\n" );

	}

	half_order = ( int ) floor( mxGetScalar( prhs[4] ) );

	if ( half_order < 1 ) {

		mexErrMsgTxt( "5th rhs argument must be >= 1 \n" );

	}

	if ( 2*half_order > num_points ) {


		mexErrMsgTxt( "5th rhs argument too large \n" );

	}

	/* Get 6th rhs argument */
	if ( !mxIsDouble( prhs[5] ) || mxIsComplex( prhs[5] ) ) {

		mexErrMsgTxt( "6th rhs argument of incorrect type\n" );
	...
	if ( mxGetN( prhs[5] ) != 1 || mxGetM( prhs[5] ) != 1 ) {
		...
		mexErrMsgTxt( "6th rhs argument of incorrect size\n" );

	}

	deriv_order = ( int ) mxGetScalar( prhs[5] );


	if ( deriv_order < 0 ) {

		mexErrMsgTxt( "6th rhs argument must be > 0 \n" );

	}

	if ( deriv_order > 2*half_order ) {

		mexErrMsgTxt( "6th rhs argument too large \n" );

	}

	/* Get 7th argument if required, otherwise set var = -1 */
	if ( nrhs == 7 ) {

		if ( !mxIsDouble( prhs[6] ) || mxIsComplex( prhs[6] ) ) {

			mexErrMsgTxt( "7th rhs argument of incorrect type\n" );
		...
		if ( mxGetN( prhs[6] ) != 1 || mxGetM( prhs[6] ) != 1 ) {
			...
			mexErrMsgTxt( "7th rhs argument of incorrect size\n" );

		}

		var = mxGetScalar( prhs[6] );
	...
	/* Allocate space for spline coefficients */
	spline_coeff = ( double* ) mxCalloc( num_points, sizeof( double ) );


	/* Allocate work vector */
	work_vector = ( double* ) mxCalloc( 
		6*( num_points*half_order + 1 ) + num_points, 
		sizeof( double ) 
		);
	...
	if ( ier == 1 ) {
		...
		mexErrMsgTxt( "GCVSPL ERROR: half_order < 0 or 2*half_order > num_points \n" );

	}
	
	if ( ier == 2 ) {

		mexErrMsgTxt( "GCVSPL ERROR: knot sequence is not strictly increasing \n" );

	}

	/* Allocate output array */
	plhs[0] = mxCreateDoubleMatrix( num_out, 1, mxREAL );

	y_new = mxGetPr( plhs[0] );

	/* Allocate second work vector */
	work_vector2 = mxCalloc( 2*half_order, sizeof( double ) );

	/* Calculate new values */
	for ( i = 0; i < num_out; i++ ) {

		/* Check that value is in range */
		if ( x_new[i] < x[0] || x_new[i] > x[num_points - 1] ) {

			mexErrMsgTxt( "3rd rhs argument - array values out of range \n" );
	...
	/* Allocate 2nd lhs arg if nrhs == 6 */
	if ( nrhs == 6 && nlhs == 2 ) {

		plhs[1] = mxCreateDoubleMatrix( 1, 1, mxREAL );

		var_out_pr = mxGetPr( plhs[1] );
